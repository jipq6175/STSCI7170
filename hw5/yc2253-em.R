...
vcem = function(X, Y, zlist, beta, u, tol=1e-5, maxiter=10000){
  ...
  while((d = tol) & (counter < maxiter)){
    ...
    for(i in 1:q){
      ...
      #                  + u0[i]^2 * t(b) %*% (Z %*% b) )/r[i]; 
      u0[i] = ( u0[i] * r[i] + u0[i]^2 * t(b) %*% (Z %*% b) )/r[i]; 
    }
    ...
    if(counter %% 1 == 0) {
      ...
      print(sprintf(fmt = "   Marginal LL = %0.4f ..", d));
    }
