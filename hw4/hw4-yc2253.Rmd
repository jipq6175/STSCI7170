...
\frac{1}{\sqrt{t}} & 0 & 0 & \dots & -\frac{t-1}{\sqrt{n(n-1)}} \end{bmatrix} = \begin{bmatrix} p_1 & p_2 & \dots & p_t \end{bmatrix}$$
...
$$Y_i = (\mu_0 + \gamma z_i)\mathbf{1}_t + B_i\mathbf{1}_t + E_i$$
where $B_i \sim N(0, \sigma_b^2)$ and $E_i \sim N_t(0, \sigma_e^2I_t)$. Therefore, $Y_i \sim N_t\left[(\mu_0 + \gamma z_i)\mathbf{1}_t, \sigma_b^2\mathbf{1}_t\mathbf{1}_t' + \sigma_e^2I_t \right] \sim N_t\left[(\mu_0 + \gamma z_i)\mathbf{1}_t, t\sigma_b^2\bar{J_t} + \sigma_e^2I_t \right]$. Let $\Sigma_i = t\sigma_b^2\bar{J_t} + \sigma_e^2I_t$ and $\Sigma_i$'s are identical for  $i=1,2,\dots,b$. 
